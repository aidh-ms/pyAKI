...
class TestKDIGO:
    def setup_test_df(
        ...
        if len(urineoutput) != desired_length or len(creatinine) != desired_length:
            raise ValueError(
                "Length of urine output or creatinine output does not match desired length"
            )

        base_date = datetime.datetime.now()
        data = {
            "charttime": [
                base_date - datetime.timedelta(hours=x) for x in range(desired_length)
            ],
    ...
    def test_kdigo_uo_criterion_all_true(self):
        df = self.setup_test_df(desired_length=50, urineoutput=70, creatinine=0.5)
    ...
    def test_kdigo_uo_criterion_all_stage_three(self):
        ...
        df = self.setup_test_df(desired_length=50, urineoutput=20, creatinine=0.5)
        df = kdigo_uo_criterion(input_ts=df, weight=70)
    ...
    def test_kdigo_uo_criterion_all_stage_two(self):
        ...
        df = self.setup_test_df(desired_length=50, urineoutput=30, creatinine=0.5)
        df = kdigo_uo_criterion(input_ts=df, weight=70)
        assert (df.kdigo_uo_criterion == 2).to_numpy().all()

    def test_kdigo_uo_criterion_stage_one(self):
        df = self.setup_test_df(
            desired_length=50, urineoutput=[24] * 5 + [70] * 45, creatinine=0.5
        )
